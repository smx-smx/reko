...
namespace Reko.Arch.NativeArchitecture
{
    ...
    public class NativeRewriterHost : MarshalByRefObject, INativeRewriterHost
    {
        ...
        public NativeRewriterHost(Dictionary<int, RegisterStorage> regs, IStorageBinder frame, IRewriterHost host, NativeTypeFactory ntf, NativeRtlEmitter m)
        {
        ...
        public HExpr EnsurePseudoProcedure(string name, BaseType dt)
        {
            var exp = host.PseudoProcedure(name, ntf.GetRekoType((HExpr)dt));
            return m.MapToHandle(exp);
        }
    }
}
